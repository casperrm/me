...
import { addTemplateTask } from "@/lib/services/project-template-service";
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return NextResponse.json({ ok: true, taskId: task.id, position: task.position });
  } catch (err) {
    if (err instanceof AuthorizationError) {
      return NextResponse.json({ error: "You don't have permission to add a task to this template." }, { status: 403 });
    }
