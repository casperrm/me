// API-contract test for POST /api/projects/[projectId]/milestones. See
...
beforeAll(async () => {
  await wipeDatabase();
  const org = await prisma.organization.create({ data: { name: "Milestone Route Test Agency" } });
  orgId = org.id;

  const owner = await prisma.user.create({ data: { email: "milestone-owner@test.example", name: "Owner", passwordHash: "irrelevant" } });
  ownerUserId = owner.id;
  await prisma.membership.create({ data: { organizationId: org.id, userId: owner.id, role: "OWNER", status: "ACTIVE" } });

  const designer = await prisma.user.create({ data: { email: "milestone-designer@test.example", name: "Designer", passwordHash: "irrelevant" } });
  ...
  const client = await prisma.client.create({ data: { organizationId: org.id, name: "Milestone Client", companyName: "Inc", services: "[]" } });
  const project = await prisma.project.create({ data: { clientId: client.id, name: "Milestone Project" } });
...
describe("POST /api/projects/[projectId]/milestones", () => {
  it("returns 401 when no one is signed in", async () => {
    ...
    const res = await POST(request({ name: "Kickoff", dueDate: "2030-01-15" }), { params: Promise.resolve({ projectId }) });
    expect(res.status).toBe(401);
  });

  it("returns 400 when name is missing", async () => {
    getCurrentActor.mockResolvedValueOnce({ user: { id: ownerUserId }, organizationId: orgId });
    const res = await POST(request({ dueDate: "2030-01-15" }), { params: Promise.resolve({ projectId }) });
    expect(res.status).toBe(400);
  });

  it("returns 400 when dueDate is missing", async () => {
    getCurrentActor.mockResolvedValueOnce({ user: { id: ownerUserId }, organizationId: orgId });
    const res = await POST(request({ name: "Kickoff" }), { params: Promise.resolve({ projectId }) });
    expect(res.status).toBe(400);
  });
  ...
  it("returns 200 and actually persists a real milestone", async () => {
    getCurrentActor.mockResolvedValueOnce({ user: { id: ownerUserId }, organizationId: orgId });
    const res = await POST(request({ name: "Kickoff", dueDate: "2030-01-15" }), { params: Promise.resolve({ projectId }) });
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body).toEqual({ ok: true, milestoneId: expect.any(String) });

    const stored = await prisma.milestone.findUnique({ where: { id: body.milestoneId } });
    expect(stored).toMatchObject({ projectId, name: "Kickoff", done: false });
  });
  ...
  it("returns 400 for a project in a different organization", async () => {
    ...
    getCurrentActor.mockResolvedValueOnce({ user: { id: ownerUserId }, organizationId: orgId });
    const res = await POST(request({ name: "Nope", dueDate: "2030-01-15" }), { params: Promise.resolve({ projectId: otherProject.id }) });
    expect(res.status).toBe(400);
  });
});
