// API-contract test for POST /api/invoices/[id]/send. See
...
beforeAll(async () => {
  await wipeDatabase();
  const org = await prisma.organization.create({ data: { name: "Invoice Send Route Test Agency" } });
  orgId = org.id;

  const owner = await prisma.user.create({ data: { email: "invoice-send-owner@test.example", name: "Owner", passwordHash: "irrelevant" } });
  ownerUserId = owner.id;
  await prisma.membership.create({ data: { organizationId: org.id, userId: owner.id, role: "OWNER", status: "ACTIVE" } });

  const designer = await prisma.user.create({ data: { email: "invoice-send-designer@test.example", name: "Designer", passwordHash: "irrelevant" } });
  ...
  const client = await prisma.client.create({ data: { organizationId: org.id, name: "Invoice Send Client", companyName: "Inc", services: "[]" } });
  clientId = client.id;
});
...
function request() {
  return new Request("http://localhost/api/invoices/x/send", { method: "POST" });
}
...
describe("POST /api/invoices/[id]/send", () => {
  ...
  it("returns 403 for a member without finance:write", async () => {
    const invoice = await prisma.invoice.create({ data: { clientId, amountCents: 10000, status: "DRAFT" } });
  ...
  it("returns 200 and actually transitions a real draft invoice to SENT", async () => {
    ...
    const timeline = await prisma.clientTimelineEvent.findFirst({ where: { clientId, type: "invoice_sent" } });
    expect(timeline).toBeTruthy();
  });

  it("returns 400 with the real service's message when the invoice isn't a draft", async () => {
    const invoice = await prisma.invoice.create({ data: { clientId, amountCents: 30000, status: "SENT" } });
    getCurrentActor.mockResolvedValueOnce({ user: { id: ownerUserId }, organizationId: orgId });
    const res = await POST(request(), { params: Promise.resolve({ id: invoice.id }) });
    expect(res.status).toBe(400);
    const body = await res.json();
    expect(body.error).toMatch(/only a draft invoice/i);
  });
  ...
  it("returns 400 for an invoice from a different organization", async () => {
    ...
    const otherInvoice = await prisma.invoice.create({ data: { clientId: otherClient.id, amountCents: 5000, status: "DRAFT" } });
