...
export async function POST(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    await sendInvoice({ actorUserId: actor.user.id, organizationId: actor.organizationId, invoiceId: id });
    return NextResponse.json({ ok: true });
  } catch (err) {
    if (err instanceof AuthorizationError) {
      return NextResponse.json({ error: "You don't have permission to send invoices." }, { status: 403 });
    }
