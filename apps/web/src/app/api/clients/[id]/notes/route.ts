...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!body?.body) return NextResponse.json({ error: "Note body is required." }, { status: 400 });
  ...
  try {
    ...
    if (err instanceof AuthorizationError) {
      return NextResponse.json({ error: "You don't have permission to add a note for this client." }, { status: 403 });
    }
