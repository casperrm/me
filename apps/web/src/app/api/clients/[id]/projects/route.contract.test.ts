// API-contract test for POST /api/clients/[id]/projects. See
...
beforeAll(async () => {
  await wipeDatabase();
  const org = await prisma.organization.create({ data: { name: "Client Project Route Test Agency" } });
  orgId = org.id;

  const owner = await prisma.user.create({ data: { email: "client-project-owner@test.example", name: "Owner", passwordHash: "irrelevant" } });
  ownerUserId = owner.id;
  await prisma.membership.create({ data: { organizationId: org.id, userId: owner.id, role: "OWNER", status: "ACTIVE" } });

  const designer = await prisma.user.create({ data: { email: "client-project-designer@test.example", name: "Designer", passwordHash: "irrelevant" } });
  ...
  const client = await prisma.client.create({ data: { organizationId: org.id, name: "Client Project Client", companyName: "Inc", services: "[]" } });
  clientId = client.id;
});
...
function request(body: unknown) {
  return new Request("http://localhost/api/clients/x/projects", {
    method: "POST",
...
describe("POST /api/clients/[id]/projects", () => {
  it("returns 401 when no one is signed in", async () => {
    ...
    const res = await POST(request({ name: "New Project" }), { params: Promise.resolve({ id: clientId }) });
  ...
  it("returns 200 and actually persists a real project", async () => {
    getCurrentActor.mockResolvedValueOnce({ user: { id: ownerUserId }, organizationId: orgId });
    const res = await POST(request({ name: "Rebrand 2026" }), { params: Promise.resolve({ id: clientId }) });
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body).toEqual({ ok: true, projectId: expect.any(String) });

    const stored = await prisma.project.findUnique({ where: { id: body.projectId } });
    expect(stored).toMatchObject({ clientId, name: "Rebrand 2026" });
  });
  ...
  it("returns 400 for a client in a different organization", async () => {
    ...
    getCurrentActor.mockResolvedValueOnce({ user: { id: ownerUserId }, organizationId: orgId });
    const res = await POST(request({ name: "Other's Project" }), { params: Promise.resolve({ id: otherClient.id }) });
    expect(res.status).toBe(400);
  });
});
