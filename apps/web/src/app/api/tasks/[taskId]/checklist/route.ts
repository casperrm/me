...
import { addTaskChecklistItem } from "@/lib/services/project-service";
...
export async function POST(req: Request, { params }: { params: Promise<{ taskId: string }> }) {
  ...
  if (!body?.text) return NextResponse.json({ error: "Checklist item text is required." }, { status: 400 });
  ...
  try {
    ...
    if (err instanceof AuthorizationError) {
      return NextResponse.json({ error: "You don't have permission to add checklist items to this task." }, { status: 403 });
    }
