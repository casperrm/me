// API-contract test for POST /api/tasks/[taskId]/checklist. See
...
beforeAll(async () => {
  await wipeDatabase();
  const org = await prisma.organization.create({ data: { name: "Task Checklist Route Test Agency" } });
  orgId = org.id;

  const owner = await prisma.user.create({ data: { email: "checklist-owner@test.example", name: "Owner", passwordHash: "irrelevant" } });
  ownerUserId = owner.id;
  await prisma.membership.create({ data: { organizationId: org.id, userId: owner.id, role: "OWNER", status: "ACTIVE" } });

  const designer = await prisma.user.create({ data: { email: "checklist-designer@test.example", name: "Designer", passwordHash: "irrelevant" } });
  ...
  const client = await prisma.client.create({ data: { organizationId: org.id, name: "Checklist Client", companyName: "Inc", services: "[]" } });
  const project = await prisma.project.create({ data: { clientId: client.id, name: "Checklist Project" } });
  const task = await prisma.task.create({ data: { projectId: project.id, title: "Checklist Task" } });
  taskId = task.id;
});
...
function request(body: unknown) {
  return new Request("http://localhost/api/tasks/x/checklist", {
    method: "POST",
...
describe("POST /api/tasks/[taskId]/checklist", () => {
  it("returns 401 when no one is signed in", async () => {
    ...
    const res = await POST(request({ text: "Draft v1" }), { params: Promise.resolve({ taskId }) });
  ...
  it("returns 200 and actually persists a real checklist item at position 0", async () => {
    getCurrentActor.mockResolvedValueOnce({ user: { id: ownerUserId }, organizationId: orgId });
    const res = await POST(request({ text: "Draft v1" }), { params: Promise.resolve({ taskId }) });
    ...
    const stored = await prisma.taskChecklistItem.findUnique({ where: { id: body.itemId } });
    expect(stored).toMatchObject({ taskId, text: "Draft v1", done: false, position: 0 });
  });
