// API-contract test for POST /api/tasks/[taskId]/dependencies. See
...
beforeAll(async () => {
  await wipeDatabase();
  const org = await prisma.organization.create({ data: { name: "Dependency Route Test Agency" } });
  orgId = org.id;

  const owner = await prisma.user.create({ data: { email: "dependency-owner@test.example", name: "Owner", passwordHash: "irrelevant" } });
  ownerUserId = owner.id;
  await prisma.membership.create({ data: { organizationId: org.id, userId: owner.id, role: "OWNER", status: "ACTIVE" } });

  const designer = await prisma.user.create({ data: { email: "dependency-designer@test.example", name: "Designer", passwordHash: "irrelevant" } });
  ...
  const client = await prisma.client.create({ data: { organizationId: org.id, name: "Dependency Client", companyName: "Inc", services: "[]" } });
  const project = await prisma.project.create({ data: { clientId: client.id, name: "Dependency Project" } });
...
function request(body: unknown) {
  return new Request("http://localhost/api/tasks/x/dependencies", {
    method: "POST",
...
describe("POST /api/tasks/[taskId]/dependencies", () => {
  it("returns 401 when no one is signed in", async () => {
    ...
    const res = await POST(request({ blockedByTaskId: blockerTaskId }), { params: Promise.resolve({ taskId }) });
    expect(res.status).toBe(401);
  });

  it("returns 400 when blockedByTaskId is missing", async () => {
  ...
  it("returns 200 and actually persists a real dependency", async () => {
    getCurrentActor.mockResolvedValueOnce({ user: { id: ownerUserId }, organizationId: orgId });
    const res = await POST(request({ blockedByTaskId: blockerTaskId }), { params: Promise.resolve({ taskId }) });
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body).toEqual({ ok: true, dependencyId: expect.any(String) });

    const stored = await prisma.taskDependency.findUnique({ where: { id: body.dependencyId } });
    expect(stored).toMatchObject({ taskId, blockedByTaskId: blockerTaskId });
  });
  ...
  it("returns 400 for a task in a different organization", async () => {
    ...
    getCurrentActor.mockResolvedValueOnce({ user: { id: ownerUserId }, organizationId: orgId });
    const res = await POST(request({ blockedByTaskId: blockerTaskId }), { params: Promise.resolve({ taskId: otherTask.id }) });
    expect(res.status).toBe(400);
  });
});
