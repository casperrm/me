...
import { addMeetingFollowUp } from "@/lib/services/meeting-service";
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!body?.text) return NextResponse.json({ error: "Follow-up text is required." }, { status: 400 });
  ...
  try {
    const followUp = await addMeetingFollowUp({
      ...
      meetingId: id,
      text: body.text,
      ownerMembershipId: body.ownerMembershipId || undefined,
      dueDate: body.dueDate ? new Date(body.dueDate) : undefined,
    });
    return NextResponse.json({ ok: true, followUpId: followUp.id });
  } catch (err) {
    if (err instanceof AuthorizationError) {
      return NextResponse.json({ error: "You don't have permission to add a follow-up to this meeting." }, { status: 403 });
    }
