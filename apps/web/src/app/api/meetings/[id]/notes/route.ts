...
import { updateMeetingNotes } from "@/lib/services/meeting-service";
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (typeof body?.notes !== "string") return NextResponse.json({ error: "Notes text is required." }, { status: 400 });
  ...
  try {
    ...
    if (err instanceof AuthorizationError) {
      return NextResponse.json({ error: "You don't have permission to edit this meeting." }, { status: 403 });
    }
