// API-contract test for POST /api/integrations/connections/[id]/revoke.
...
beforeAll(async () => {
  await wipeDatabase();
  const org = await prisma.organization.create({ data: { name: "Connection Revoke Route Test Agency" } });
  orgId = org.id;

  const owner = await prisma.user.create({ data: { email: "connection-revoke-owner@test.example", name: "Owner", passwordHash: "irrelevant" } });
  ownerUserId = owner.id;
  await prisma.membership.create({ data: { organizationId: org.id, userId: owner.id, role: "OWNER", status: "ACTIVE" } });

  const designer = await prisma.user.create({ data: { email: "connection-revoke-designer@test.example", name: "Designer", passwordHash: "irrelevant" } });
  ...
  const { connectionId: id } = await createGenericWebhookConnection({ actorUserId: ownerUserId, organizationId: orgId, name: "Test Connection" });
  connectionId = id;
});
...
function request() {
  return new Request("http://localhost/api/integrations/connections/x/revoke", { method: "POST" });
}

describe("POST /api/integrations/connections/[id]/revoke", () => {
  it("returns 401 when no one is signed in", async () => {
    getCurrentActor.mockResolvedValueOnce(null);
    const res = await POST(request(), { params: Promise.resolve({ id: connectionId }) });
  ...
  it("returns 400 for a connection in a different organization", async () => {
    const otherOrg = await prisma.organization.create({ data: { name: "Other Org" } });
    const otherOwner = await prisma.user.create({ data: { email: "other-owner-revoke@test.example", name: "Other Owner", passwordHash: "irrelevant" } });
    await prisma.membership.create({ data: { organizationId: otherOrg.id, userId: otherOwner.id, role: "OWNER", status: "ACTIVE" } });
    const { connectionId: otherConnectionId } = await createGenericWebhookConnection({
    ...
    const res = await POST(request(), { params: Promise.resolve({ id: otherConnectionId }) });
    expect(res.status).toBe(400);
  });

  it("returns 200 and actually marks the connection DISCONNECTED", async () => {
    getCurrentActor.mockResolvedValueOnce({ user: { id: ownerUserId }, organizationId: orgId });
    const res = await POST(request(), { params: Promise.resolve({ id: connectionId }) });
    ...
    const stored = await prisma.connection.findUnique({ where: { id: connectionId } });
