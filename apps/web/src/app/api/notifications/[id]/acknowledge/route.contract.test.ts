// API-contract test for POST /api/notifications/[id]/acknowledge. See
...
beforeAll(async () => {
  await wipeDatabase();
  const org = await prisma.organization.create({ data: { name: "Notification Ack Route Test Agency" } });
  orgId = org.id;

  const owner = await prisma.user.create({
    data: { email: "notif-ack-owner@test.example", name: "Owner", passwordHash: "irrelevant" },
  });
  ...
  const other = await prisma.user.create({
    data: { email: "notif-ack-other@test.example", name: "Other", passwordHash: "irrelevant" },
  });
...
function request() {
  return new Request("http://localhost/api/notifications/x/acknowledge", { method: "POST" });
}
...
describe("POST /api/notifications/[id]/acknowledge", () => {
  ...
  it("returns 400 for a notification that doesn't belong to the caller's own membership", async () => {
    const notification = await prisma.notification.create({
      data: { organizationId: orgId, membershipId: otherMembershipId, severity: "CRITICAL", category: "test", title: "For someone else" },
    });
  ...
  it("returns 200 and actually marks the caller's own notification ACKNOWLEDGED", async () => {
    const notification = await prisma.notification.create({
      data: { organizationId: orgId, membershipId: ownerMembershipId, severity: "CRITICAL", category: "test", title: "For the owner" },
    });
    getCurrentActor.mockResolvedValueOnce({ user: { id: ownerUserId }, organizationId: orgId, membership: { id: ownerMembershipId } });
    const res = await POST(request(), { params: Promise.resolve({ id: notification.id }) });
